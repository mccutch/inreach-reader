...
export class GenericNavbar extends React.Component {
  ...
  render() {
    ...
    if (this.props.app.loggedIn) {
      ...
      navRight = <ul className="navbar-nav mr-auto mt-2 mt-lg-0">{logout}</ul>;
    } else {
      navLeft = (
        <ul className="navbar-nav mr-auto mt-2 mt-lg-0">
          {home}
    ...
    let navColour = this.props.app.serverError
      ? ERROR_COLOUR
...
export class BootstrapNavBar extends React.Component {
  render() {
    return (
      <nav
        className={`navbar navbar-expand-lg ${this.props.styleString} bg-${this.props.navColour}`}
      >
        <button className="navbar-brand btn bg-transparent">
          <img alt="" src={this.props.icon} width="40" />{" "}
          <CleanLink
            className="text-light"
