...
import { displayISODate, displayISOTime } from "./dateFunctions";
    ...
    for (let i in this.props.actions) {
      actionList.push(
        <Dropdown.Item
          key={i}
          as="button"
          onClick={() => this.props.actions[i].action(this.props.object)}
        >
          {this.props.actions[i].label}
        </Dropdown.Item>
      );
    }
    return actionList;
    ...
    return (
      <button
        className={`${
          this.props.noFormat
            ? "btn btn-outline"
            : "btn btn-outline-primary btn-block my-1"
        }`}
      ...
      >
        <div className="row" style={{ height: "2.5rem" }}>
          ...
          <div className="col-10">
            <div className="row">
              <div
                className="col text-truncate text-left"
                style={{ margin: "0rem 0rem 0rem -0.25rem" }}
              >
            ...
            <div className="row" style={{ height: "1rem" }}>
              <div
                className="col text-truncate text-left"
                style={{ margin: "-0.5rem 0rem 0rem -0.25rem" }}
              >
  ...
  render() {
    ...
    return (
      <DropdownActionsButton
        ...
        secondaryText={`${displayISODate(trip.departs)}-${displayISODate(
          trip.returns
        )}`}
        primaryRight={``}
        secondaryRight={``}
