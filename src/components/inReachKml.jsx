    ...
    getKMLData({
      ...
      onSuccess: (json) =>
        this.setState({
          ...
          tested: true,
          successMessage:
            "Mapshare ID received a valid response from Garmin services.",
        }),
      onFailure: (err) =>
        this.setState({
          isValid: false,
  ...
  render() {
      ...
      <div>
        ...
        <input
          ...
          className={`form-control my-2 ${
            this.state.isValid ? "is-valid" : ""
          } ${this.state.tested && !this.state.isValid ? "is-invalid" : ""}`}
        />
        <small
          className={`${
            !this.state.isValid ? "invalid-feedback" : "valid-feedback"
          }`}
        >
          {this.state.tested ? (
            <div>
      ...
      <div>
        {this.state.isValid && (
          <button
            ...
            onClick={() => this.saveChanges(this.state.ID_input)}
          >
            Save changes
          </button>
        )}
        {this.props.user.profile.mapshare_ID && (
          <button
    ...
    return (
      <StandardModal
        ...
        body={body}
        footer={footer}
        hideModal={this.checkBeforeClose}
      />
    );
  }
}

export function getKMLData({
  mapshareID,
  startDate,
  endDate,
  onSuccess,
