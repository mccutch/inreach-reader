...
function WarningModal({ body, warnings, hideModal, onContinue }) {
  ...
  let footer = (
    <div>
      <button className="btn btn-outline-danger m-2" onClick={hideModal}>
        Cancel
      </button>
      <button className="btn btn-outline-success m-2" onClick={onContinue}>
        Continue
      </button>
    </div>
  );
...
function PendingBtn({ name, onClick, className, pending, disabled, children }) {
  ...
  return (
    <button
      ...
      className={`btn ${className} ${inactive ? "disabled" : ""}`}
    >
      {children}
    </button>
  );
...
function CenterPage({ children }) {
  return (
    <div className="container-sm my-2 bg-light" style={{ maxWidth: "600px" }}>
      {children}
    </div>
  );
}
...
function VerticalSpacer({ height }) {
  return (
    <div>
      <div
        className="col"
        style={{ height: `${height ? height : 3}rem` }}
      ></div>
    </div>
  );
}
...
function StandardModal({ title, body, footer, headerClass, hideModal }) {
  return (
    <Modal show={true} onHide={hideModal}>
      <Modal.Header
        className={headerClass ? headerClass : "bg-teal text-light"}
        closeButton
      >
...
function FormRow({ labelWidth, label, input, helpText }) {
  ...
  return (
    <div className="form-group form-row">
      ...
      {helpText ? (
        <small className="form-text text-muted mx-2">{helpText}</small>
      ) : (
        ""
      )}
    </div>
    ...
    for (let i in tabData) {
      navBar.push(
        <li className="nav-item">
          {this.makeTab(i.toString(), tabData[i].label)}
        </li>
      );
    }
    return navBar;
...
function ObjectSelectionList({
  ...
  for (let i = 0; i < selections.length; i++) {
    listOptions.push(
      <option value={selections[i][value]} key={key ? selections[i][key] : i}>
