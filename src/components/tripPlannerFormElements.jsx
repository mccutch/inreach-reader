  ...
  function editDateTime(){
    app.setModal(
      <TimeInputModal
        ...
        app={app} 
      >{children}</TimeInputModal>
    )
  }

  return(
    <button className={`btn text-left text-light btn-info btn-block my-2`} onClick={editDateTime}>
      {children}: <em>{df.displayDate(value)}, {df.displayTime(value)}</em>
    </button>           
  )
}
...
function TimeInputModal({
  ...
  const body = 
    <form className="form-row">
      <div className="col-6">
        <input className="form-control my-2" type="date" defaultValue={date} onChange={(e)=>setDate(e.target.value)}/>
      </div>
      <div className="col-6">
        <input className="form-control my-2" type="time" defaultValue={time} onChange={(e)=>setTime(e.target.value)}/>
      </div>
    </form>

  const footer = <button className="btn btn-block btn-success" onClick={formatDateTime}>Continue</button>  
  return <StandardModal title={title} body={body} footer={footer} hideModal={app.hideModal}/>
}
