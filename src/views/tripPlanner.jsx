...
function TripPlanner({ app, user, trip }) {
  ...
  function validateInputs() {
    ...
    if (d.returnTime < d.departTime)
      return returnError("Return time must be later than departure.");
    if (d.overdueTime && d.overdueTime < d.returnTime)
      return returnError("Overdue time must be later than return time.");
    if (d.overdueTime && !d.overdueInstructions)
      return returnError(
        "Please provide instructions in case you are overdue."
      );
...
function isDescriptionProvided(d) {
  if (!d.description) {
    return "No trip description provided.";
  }
}
...
function isOverdueTimeValid(d) {
  if (
    d.overdueTime &&
    (d.overdueTime.getTime() - d.returnTime.getTime()) / (1000 * 60 * 60) < 2
  ) {
