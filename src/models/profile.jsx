  ...
  render() {
    ...
    return this.state.edit ? (
      <ProfileEdit
        ...
        completeEdit={() => this.setState({ edit: false })}
      />
    ) : (
      <div>
        ...
        <p>
          Link:{" "}
          <strong>
            {window.location.host}/#/view/{user.username}
          </strong>
        </p>
        <p>
          Pass phrase:{" "}
          <strong>
            <em>{profile.pass_phrase}</em>
          </strong>
        </p>
        <button
        ...
        <button
          ...
          onClick={() => this.setState({ redirect: urls.HOME })}
        >
          Back
        </button>
        <h4>inReach Feed</h4>
        <div>
          <p>
            Mapshare ID:{" "}
            <strong>
              <em>{profile.mapshare_ID || "Not linked"}</em>
            </strong>
          </p>
          <button
            className="btn btn-outline-success m-2"
        ...
        <ContactList
          ...
          actions={[
            ...
            },
            {
              label: "Say hi!",
              action: (contact) => {
                console.log(`Hi ${contact.first_name}!`);
              },
            },
          ]}
        />
        <button
          ...
          onClick={() => {
            this.props.app.setModal(
              <EditContact user={this.props.user} app={this.props.app} />
            );
          }}
        >
          + Add
...
export class ProfileEdit extends React.Component {
    ...
    if (willUpdateUser) {
      ...
      apiFetch({
        ...
        onSuccess: () => {
          ...
          this.setState({ userUpdatePending: false }, this.checkUpdateComplete);
        },
  ...
  handlePatchFailure(errorMessage) {
    this.setState({
      errorMessage: errorMessage ? errorMessage : "Unable to save changes.",
    });
  }
      ...
      <div>
        ...
        <GoogleAutocomplete
          ...
          className={`form-control my-2 ${
            this.state.locationData ? "is-valid" : ""
          }`}
        ...
        <input
          name="pass_phrase"
          type="text"
          defaultValue={profile.pass_phrase}
          placeholder="Profile pass phrase"
          maxLength={MAX_LEN_NAME}
          onChange={this.handleChange}
        />
        <br />
        <br />
        <button className="btn btn-outline-success" onClick={this.validateData}>
          Save changes
        </button>
        <button
          className="btn btn-outline-danger"
          onClick={this.props.completeEdit}
        >
          Cancel
        </button>
      </div>
