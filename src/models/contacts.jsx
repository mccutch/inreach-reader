...
function AddEmergencyContactButton(props) {
  return (
    <button
      ...
      onClick={() => renderEmergencyContactSelection(props)}
    >
...
function renderEmergencyContactSelection({ app, user, onSelect }) {
  ...
  app.setModal(
    <StandardModal title={title} body={body} hideModal={app.hideModal} />
  );
}
...
function ViewContact({ contact, app, user }) {
  ...
  let footer = app.loggedIn ? (
    <button
      ...
      onClick={() => {
        app.setModal(<EditContact contact={contact} user={user} app={app} />);
      }}
    >
      Edit
    </button>
...
class EditContact extends React.Component {
  ...
  saveContact() {
    ...
    apiFetch({
      url: saveNew
        ? urls.MY_EMG_CONTACTS
        : `${urls.EMG_CONTACT}/${this.props.contact.id}/`,
  ...
  render() {
    ...
    let body = (
      <div>
        ...
        />
        <p>
          Note: All information stored here will be visible to anyone who views
          your trip.
        </p>
        <p>
    ...
    let footer = !this.state.attemptDelete ? (
      <div>
        <button
          ...
          onClick={this.props.app.hideModal}
        >
          Cancel
        </button>
        {this.props.contact && (
          <button
            ...
            onClick={() => this.setState({ attemptDelete: true })}
          >
            Delete
          </button>
        )}
        <button
          className={`btn btn-success m-2 ${
            this.state.submissionPending ? "disabled" : ""
          }`}
      ...
      <div>
        ...
        <button
          className="btn btn-outline-danger m-2"
          onClick={() => this.setState({ attemptDelete: false })}
        >
          Cancel
        </button>
        <button className="btn btn-dark m-2" onClick={this.deleteContact}>
          Confirm
        </button>
      </div>
    );
